...
def parse_job_dt(raw: str) -> datetime | None:
    ...
    - formato viejo:  2026-02-12 02:48
    """
    if not raw:
        return None
...
def is_due(job_dt: datetime, now_dt: datetime) -> bool:
    """
    Compara bien tanto aware vs naive.
    """
    if job_dt.tzinfo is None and now_dt.tzinfo is not None:
        # si el job es naive pero now es aware, volvemos now naive
        return job_dt <= now_dt.replace(tzinfo=None)
    if job_dt.tzinfo is not None and now_dt.tzinfo is None:
        # si el job es aware pero now es naive, volvemos job naive
        return job_dt.replace(tzinfo=None) <= now_dt
    return job_dt <= now_dt


def send_job(cfg: dict, to_addr: str, subject_override: str | None = None, template_override: str | None = None) -> None:
    """
    Envía 1 correo a 1 destinatario usando tu lógica del 010 para construir el mensaje.
    Lanza excepción si falla para poder marcar failed/retry.
    """
    smtp_cfg = cfg["smtp"]
    host = smtp_cfg["host"]
    ...
    try:
        msg = sender.create_message_for_recipient(cfg, to_addr)
        ...
        # log OK (usamos tu logger del 010)
        ...
        # restaurar config (para no “contaminar” siguientes jobs)
...
def bump_retry(cfg: dict, job: dict, err: str) -> None:
    app = cfg.get("app", {})
    max_retries = int(app.get("max_retries", 2))          # opcional
    retry_delay = int(app.get("retry_delay_seconds", 300)) # opcional
...
if __name__ == "__main__":
    ...
    qp = queue_path(cfg)
    tick = int(cfg.get("app", {}).get("tick_seconds", 5))           # opcional
    rate_limit = int(cfg.get("app", {}).get("rate_limit_seconds", 15)) # opcional
    ...
    while True:
        ...
        for job in jobs:
            ...
            try:
                send_job(cfg, to_addr, subject_override, template_override)
                job["status"] = "sent"
                job["sent_at"] = now_local(cfg).isoformat()
                job["last_error"] = ""
                changed = True
                print(f"[OK] Enviado a {to_addr}")
            ...
            # guardar después de cada envío para no perder estado
