...
# ---------- PS (new functionality #8) ----------

def pick_random_ps(cfg: dict) -> str:
    """
    Elige una frase PS aleatoria desde cfg["ps"]["phrases"].
    Devuelve "" si está desactivado o no hay frases.
    """
    ps_cfg = cfg.get("ps", {})
...
def apply_ps_to_text(body_text: str, ps_line: str, cfg: dict) -> str:
    """
    Si cfg["ps"]["add_to_text"] es true, agrega el PS al texto plano.
    """
    if not ps_line:
        return body_text
...
def apply_ps_to_html(html: str, ps_line: str, cfg: dict) -> str:
    """
    Si cfg["ps"]["add_to_html"] es true, inserta el PS al final del <body>.
    - Si existe {{PS}}, lo reemplaza.
    - Si no, lo inserta con un bloque simple con estilos inline.
    """
    if not ps_line:
        return html
    ...
    # Si por alguna razón no hay body, devolvemos tal cual
    if body is None:
        return html

    # Estilos inline (compatibles con clientes de email)
    style = ps_cfg.get(
        "html_style",
...
def create_message(cfg: dict) -> EmailMessage:
    ...
    file_attachments = load_file_attachments_from_config(cfg, Path("adjuntos"))
...
if __name__ == "__main__":
    config_path = Path("config6.json")  # nuevo config con PS
