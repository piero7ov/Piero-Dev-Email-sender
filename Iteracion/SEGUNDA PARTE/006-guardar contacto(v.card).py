...
def build_vcard(
    ...
    - phone y location son opcionales.
    """
    lines = [
        "BEGIN:VCARD",
        "VERSION:3.0",
    ...
    # Si hay teléfono, lo añadimos
    if phone:
        # Normalizamos un poco (sin obligar formato)
        lines.append(f"TEL;TYPE=CELL:{phone}")

    # Si hay ubicación, lo ponemos como NOTE simple
    if location:
        lines.append(f"NOTE:Ubicación - {location}")

    lines.append("END:VCARD")
    lines.append("")  # línea final
...
def attach_vcard(msg: EmailMessage, cfg: dict) -> None:
    ...
    # si phone viene vacío "" lo tratamos como None
...
def create_message(cfg: dict) -> EmailMessage:
    ...
    # 1) Prepare HTML (with cid:...) and list of inline image attachments
    ...
    # 4) Load file attachments from "adjuntos" and attach them
...
def send_email(cfg: dict) -> None:
    ...
    try:
        ...
        print("Email enviado.")
        log_email_result(cfg, to_for_log, subject_for_log, True, "Sent successfully (with vCard)")
